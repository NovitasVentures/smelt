...
| `fault-cleared-outside-reset` | 10 (2026-07-05, laundering fix — found by the end-to-end re-run) | 29/29, confirmed no regression | The fresh end-to-end re-run (post-build-9, `smelt_output/20260705_215725`) surfaced a real false NEGATIVE the 28-case suite had never exercised: iteration 12's generator satisfied the new `cleared_value_patterns` gate while committing the exact violation the rule exists to prevent — `FaultType fault = FaultType::NONE; ... /* branches set fault to a fault value, or leave it NONE */ ... faults_.at(cell) = fault;` routes the cleared value through a local variable before assigning it to the member, semantically identical to an else-clear but with zero literal `MEMBER = FaultType::NONE` adjacency. Root cause: build 9's "False is authoritative" reasoning for `_cleared_value_assignment_verdict` was wrong in general — it only holds when there is *also* no local-variable-to-member data-flow path, which pattern matching cannot rule out by absence alone. Verified this wasn't a hypothetical concern: fed directly to the model, this exact chunk scores 99.6% positive — the trained specialist handles the data-flow judgment correctly; the deterministic tier was the thing suppressing it. Fixed by making `_cleared_value_assignment_verdict` genuinely three-state: True (direct/indexed/range-loop member assignment — violation, no model), **None (a local variable initialized to the cleared value is later assigned to a member — defer to the model, cannot resolve by pattern alone)**, False (no member assignment and no such laundering path — clean, no model). Added `pos__shape11_laundered_else_clear.cpp` as a permanent regression case (this exact shape, correctly falls through to the model and scores positive). This is the second time in one day a "the model verdict is redundant, pattern matching is sufficient" assumption for this specialist turned out to be wrong on contact with real generated code — the lesson isn't "prefer deterministic checks," it's "deterministic checks must return 'no opinion' whenever they cannot see far enough to be sure, and the model must stay wired as a genuine fallback, not a formality." |
